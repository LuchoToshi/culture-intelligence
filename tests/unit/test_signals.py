...
def test_linking_to_existing_signal_accumulates(session):
    ...
    provider2 = FakeProvider([SignalMatchResponse(links=[link(second.id, existing=signal.id)])])
...
def test_registry_passed_to_prompt(session):
    ...
    SignalService(
        session,
        FakeProvider([SignalMatchResponse(links=[link(first.id, new_name="Named Signal")])]),
    ).update_signals()

    make_analyzed_item(session, source, "Later Item")
    provider = FakeProvider([SignalMatchResponse(links=[])])
    SignalService(session, provider).update_signals()
